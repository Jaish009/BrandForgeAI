          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-8">
            {isLoading ? (
              ...
              catalogProducts.map((product) => (
                <Card key={product.id} className="border-none! shadow-lg ring-0 group aspect-[5/6] p-0 rounded-xl cursor-pointer">
                  <Link to={PROTECTED_ROUTES.DESIGN.replace(":product_id", product.id)} className="relative h-full  overflow-hidden rounded-xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {isLoading ? (
              ...
              featuredProducts.map((product: any) => (
                <Card key={product.id} className="border-none! shadow-sm p-0 rounded-lg group cursor-pointer overflow-hidden">
                  <Link to={PROTECTED_ROUTES.DESIGN.replace(":product_id", product.id)}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {isLoadingListings ? (
              ...
              listings.slice(0, 8).map((listing: any) => (
                <Card key={listing.id} className="border-none! shadow-sm p-0
                    ...
                    <div className="px-3 py-3 space-y-1">
                      ...
                      <div className="flex gap-1">
                        {listing.colorIds?.map((color: any) => (
                          <div key={color.id} className="size-4 rounded-full border"
